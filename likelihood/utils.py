...
def array_to_dict(arr, parameters):
    ...
    Args:
        arr (np.array): Array of input values

    Returns:
        dict: Dictionary with parameter names as keys
    """
    result_dict = {key: arr[i] for i, key in enumerate(parameters)}
    result_dict["luminosity_distance"] = 44.0
    result_dict["timeshift"] = 0.0
...
def log_likelihood(parameters, 
    ...
    mag_app_dict = calc_lc_given_params_flax(parameters, times, luminosity_distance = luminosity_distance)
    ...
    for filt in filters:
        ...
        chisq_filt = get_chisq_filt(mag_est_filt, sample_times, data_time, data_mag, data_sigma, t0=t0)
...
# Note: both are given the parameters
likelihood_fn_flax = lambda x: log_likelihood(x, sample_times, data, trigger_time)

nmma_likelihood = OpticalLightCurve(lc_model_tensorflow,
                                    filters,
                                    data,
                                    trigger_time,
                                    )
...
def likelihood_fn_nmma(x):
    nmma_likelihood.parameters = array_to_dict(x)
